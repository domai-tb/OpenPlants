...
**Reason**: The plant collection page is removed as a standalone page. The plant grid moves to the unified dashboard, which becomes the sole surface for viewing all plants. The grid layout and search/filter capabilities are preserved under the unified-dashboard spec.

**Migration**: Reference `unified-dashboard` spec for the plant grid requirement. The `PlantCollectionPage` widget is deleted. Grid rendering logic is extracted into a shared widget used by the dashboard.

### Requirement: User can filter plants by room
**Reason**: Room filter chips move to the unified dashboard. The behavior is preserved.

**Migration**: Reference `unified-dashboard` spec for filter requirements.

### Requirement: User can search plants by name
**Reason**: Search functionality moves to the unified dashboard. The behavior is preserved.

**Migration**: Reference `unified-dashboard` spec for search requirements.
...
The system SHALL allow users to create a new plant entry with name (required), optional photo, optional species, optional room (selected from defined rooms), optional notes, initial care status (default: happy), and an empty photo timeline. The add-plant form SHALL be accessible from the dashboard's quick-action "Add Plant" button and from the onboarding empty state.
