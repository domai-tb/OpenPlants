...
The system SHALL display the plant identification flow as a full-screen modal dialog rather than a top-level page. The identification modal SHALL contain a camera/gallery capture area and a results area.

#### Scenario: Identification triggered from dashboard
- **WHEN** the user taps "Identify" on the dashboard quick-action strip
- **THEN** the system opens a full-screen modal showing the capture prompt with camera and gallery buttons

#### Scenario: Initial state in modal
- **WHEN** the identification modal opens
- **THEN** the user sees a capture prompt with camera and gallery buttons, and no results are shown
