# Early Completion Grace

Grace-window detection for tasks completed ahead of their schedule.

## Purpose

Provide a deterministic pure function that determines whether a recently completed care task was finished early (within the current calendar day with a future next-due date), enabling the UI to display such tasks in a separate Completed Early section rather than mixing them with upcoming tasks.
