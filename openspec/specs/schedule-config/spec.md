...
Per-plant and per-room configuration that feeds the care schedule engine.

## Purpose

Store user-editable settings — task interval overrides, pot type, room attributes, species profiles, and custom task types — that determine how the engine computes effective intervals.
