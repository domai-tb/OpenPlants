...
Defines the privacy information displayed during OpenPlant's first-run onboarding flow, ensuring every new user understands the app's local-first, privacy-friendly architecture before entering the app.
...
- **WHEN** the privacy page is rendered
- **THEN** a row or card SHALL display with heading "No Account Required" and an appropriate icon (e.g. `Icons.person_off_outlined`)
